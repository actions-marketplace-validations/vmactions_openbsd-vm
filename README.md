...
>
> Just describe what you want in plain language, e.g. *"run my tests on OpenBSD"* or *"check that my project builds on OpenBSD aarch64"*, and the agent generates a correct, ready-to-commit `test.yml`. It will:
>
...
It uses [the OpenBSD 7.9](conf/default.release.conf) by default, you can use `release` option to use another version of OpenBSD:
...
Here `release: "7"` runs the newest `7.x` release of OpenBSD. Each part you give has to match in full, so a release that does not exist fails the job instead of quietly falling back to another one.
...
We use Qemu to run the OpenBSD VM.
